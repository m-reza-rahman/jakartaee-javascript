...
function ChatPage({ username }: ChatPageProps) {
  ...
  return (
    <div className="p-3">
      <Card.Root className="chat-card shadow-1">
        <Card.Header>
          <Card.Title>{`Chat as ${username || 'Anonymous'}`}</Card.Title>
          <Card.Subtitle>
            <span className="text-muted">
              WebSocket communication with the backend chat endpoint.{' '}
              <span className={`badge ms-2 ${status === 'Connected' ? 'bg-success' : 'bg-secondary'}`}>
                {status}
              </span>
            </span>
          </Card.Subtitle>
